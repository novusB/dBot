...
class ToxicView(BaseView):
    """
    An interactive view for handling the toxic user vote.
    """
    def __init__(
        self,
        bot: Red,
    ...
    async def vote(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        button.label = (
            ...
            .replace("{timeout}", f"{self.settings['timeout'] // 60}m" if self.settings['timeout'] % 60 == 0 else f"{self.settings['timeout']}s")
        )
    ...
    async def on_timeout(self):
        ...
        if len(self.votes) >= self.settings["votes_needed"]:
            ...
            reason_for_log = (
                ...
                f"for `{self.reason or 'being a toxic player'}` with {len(self.votes)} out of {self.settings['votes_needed']} votes."
            )
            ...
            try:
                ...
                case = await modlog.create_case(
                    ...
                    "Toxic", # Consistent casetype name
