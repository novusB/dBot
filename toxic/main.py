...
class Toxic(commands.Cog):
    """
    A custom cog for Red Discord Bot that allows users to vote to punish toxic members.
    """

    def __init__(self, bot: Red):
        """
        Constructor for the Toxic cog.
        Args:
            bot: The Red bot instance.
        """
        self.bot = bot
        # Initialize the Config object for this cog with a unique identifier.
    ...
    async def cog_load(self):
        ...
        case_type = {
            "name": "Toxic",  # Name of the casetype
            ...
            "case_str": "Toxic", # String representation for the casetype in modlog
        }
        try:
    ...
    async def toxic(self, ctx: commands.Context, user: discord.Member, *, reason: str):
        """
        Starts a vote to punish a toxic user in the server.
        ...
        Usage:
        [p]toxic <@user_mention_or_id> <reason_for_vote>

        Examples:
        [p]toxic @AnnoyingUser spamming chat with nonsense
        [p]toxic 123456789012345678 constantly being disruptive
        """
        ...
        view = ToxicView(self.bot, settings, ctx.author, user, reason)
    ...
    async def vs(self, ctx: commands.Context):
        """
        Change the Toxic Player Punishment Settings for this server.
        """
        settings: GuildSettings = await self.config.guild(ctx.guild).all()
    ...
    async def vs_game_roles(
        ...
        Examples:
        [p]toxicsettings gameroles @GameRole1 @GameRole2
        [p]toxicsettings gameroles (to clear roles)
        """
        if not roles:
    ...
    async def vs_action(self, ctx: commands.Context, action: Literal["kick", "ban"]):
        """
        Change the action to take on a toxic user if the vote passes.
    ...
    async def vs_button(self, ctx: commands.Context):
        """
        Change the button settings for starting a toxic user vote.
        """
        settings: GuildSettings = await self.config.guild(ctx.guild).all()
        ...
        try:
            ...
            self.bot.logger.error(f"Toxic cog button preview error: {e}")
    ...
    async def vs_button_style(
        ...
        """
        Change the style (color) of the vote button.

        1: blurple, 2: grey, 3: green, 4: red.
        """
        await self.config.guild(ctx.guild).button.style.set(style)
        await ctx.send(f"Successfully set button style to {discord.ButtonStyle(style).name.capitalize()}.")
    ...
    async def vs_button_label(
        ...
        """
        Change the label (text) of the vote button.
        ...
        await ctx.send(f"Successfully set button label to: `{label}`.")
    ...
    async def vs_button_emoji(
        ...
        """
        Change the emoji displayed on the vote button.
